...
void trans_conv_cl(float * input, int W, int H, int C, float * filter, int w, int h, int c, float * output, int s){
    ...
    fp = fopen("trans_conv_kernel_v2.cl", "r");
    if (!fp) {
    ...
    ret = clSetKernelArg(kernel, 10, sizeof(cl_float) * c * h * w, NULL);
    
    // Execute the OpenCL kernel on the list
    size_t global_item_size[3] = {H * s * c, W * s * h, w};
    size_t local_item_size[3] = {c, h, w};
